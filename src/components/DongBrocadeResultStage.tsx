...
type DongBrocadeResultStageProps = {
...
const COPY = {
  zh: {
    ...
    generatingSubtitle: '结果图会按矩阵切片，从每一行的中心向两侧缓慢展开，并从顶部逐行流畅拼接成 `001.png`。',
    generatedLabel: '生成完成',
    qrTitle: '手机扫码下载',
    ...
    details: [
      '点击生成后当前页会直接跳到结果页，并先播放完整的矩阵拼接动画。',
      '当前结果资源固定使用 `result/001.png`，不会根据卡片组合变化。',
      '动画结束后会无缝停留在完整图片上，再显示二维码和下载入口。',
    ],
  },
  en: {
    ...
    generatingSubtitle:
      'The image is revealed row by row from top to bottom, and each row expands smoothly from the center outward.',
    ...
    details: [
      'Start Creating now sends the current page straight into the result route and plays the full matrix reveal animation there.',
      'The active result resource is currently fixed to `result/001.png`, independent of selected cards.',
      'After the animation, the full image stays in place without a visible switch before the QR download section appears.',
    ],
  },
} as const;
...
export default function DongBrocadeResultStage({
  cards,
  outputAsset,
  languageMode,
  generationStatus,
  ...
  return (
    <div className="min-h-screen w-full bg-stone-100 flex items-center justify-center p-3 md:p-8 lg:p-10">
      <main className="relative w-full max-w-[1320px] min-h-[840px] overflow-hidden rounded-[3.25rem] border-[10px] md:border-[12px] border-stone-900 bg-[#f7f3ec] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)]">
        ...
        <div className="relative z-10 grid gap-8 px-6 pb-8 pt-8 md:grid-cols-2 md:items-stretch md:px-8 md:pt-10">
          <section className="flex min-h-[760px] flex-col">
            ...
            <div className="relative min-h-[760px] flex-1 overflow-hidden rounded-[2rem] border border-stone-900/10 bg-[#140f12] shadow-[0_35px_70px_-30px_rgba(0,0,0,0.45)]">
              ...
              {showImageStage && (
                <div className="absolute inset-[7%] overflow-hidden rounded-[1.9rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(154,109,121,0.28),_transparent_52%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.01))]">
                  {isGenerating && (
                    <motion.div
                      className="absolute inset-x-[16%] top-0 h-24 bg-gradient-to-b from-amber-200/35 to-transparent blur-xl"
                      animate={{ y: ['-12%', '84%', '-12%'] }}
                  ...
                  <div className="absolute inset-x-0 top-8 text-center text-[10px] font-semibold tracking-[0.28em] text-white/60">
                    {isGenerating ? 'GENERATING / 001.PNG' : 'RESULT / 001.PNG'}
                  </div>

                  <div className="absolute inset-[9%] flex items-center justify-center rounded-[1.65rem] border border-white/8 bg-black/12 p-5">
                    {outputAsset ? (
                      <MatrixRevealImage
          ...
          <aside className="flex min-h-[760px] flex-col justify-between rounded-[2rem] border border-stone-900/10 bg-white/80 p-6 shadow-[0_30px_60px_-30px_rgba(0,0,0,0.3)] backdrop-blur">
            <div>
