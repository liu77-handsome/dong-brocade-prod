...
const motifMarkerModulePaths = Object.keys(
  import.meta.glob('../../纹样打标/*/TY*/*.{png,jpg,jpeg,webp,PNG,JPG,JPEG,WEBP}'),
);
...
const getFolderNameFromFinalEffectPath = (assetPath: string) => {
  const match = normalizePath(assetPath).match(/纹样打标\/([^/]+)\/最终效果\/[^/]+$/);
  return match?.[1] ?? null;
};

const getMotifCodeFromMarkerPath = (assetPath: string) => {
  const match = normalizePath(assetPath).match(/纹样打标\/([^/]+)\/(TY\d+)\/[^/]+$/);
